    ...
    ESX.TriggerServerCallback(
        ...
        function(e)
            if e then
                ...
                ESX.ShowNotification("Policia insuficiente.")
            end
        end
    )
end
StartHackingDevice = function(d)
    Citizen.CreateThread(
        function()
            cachedData["hackear"] = true
            local f = Config.Bank[d]
            if #Config.ItemNeeded > 0 then
                if not HasItem() then
                    return ESX.ShowNotification("Tu nao tens nenhum~h~~r~ " .. Config.ItemNeeded)
                end
            end
            ...
            if g ~= -1 and h <= 3.0 then
                if
                 ...
                 then
                    return ESX.ShowNotification("Outra pessoa está ~r~a utilizar o dispositivo.")
                end
            end
...
GrabCash = function(s)
    ...
    if IsEntityPlayingAnim(v, "anim@heists@ornate_bank@grab_cash", "cart_cash_dissapear", 3) then
        return ESX.ShowNotification("já estão a roubar.")
    end
                            ...
                            if G <= 1.5 then
                                if Config.BlackMoney then
                                    ESX.ShowHelpNotification(
                                        "~INPUT_DETONATE~ Apanha ~r~o dinheiro sujo ~s~do ~y~carrinho"
                                    )
                                else
                                    ESX.ShowHelpNotification("~INPUT_DETONATE~ Apanha ~g~o dinheiro ~s~do ~y~carrinho")
                                end
...
thermite = function()
    Citizen.CreateThread(
        function()
            local a3 = PlayerPedId()
            local a4 = GetEntityCoords(a3)
            if GetDistanceBetweenCoords(a4, vector3(261.58, 222.05, 106.28), true) <= 2.5 then
                SetEntityCoords(a3, vector3(261.22, 221.9, 105.4))
                SetEntityHeading(a3, 253.27)
                ...
                local thermite =
                    CreateObject(GetHashKey("hei_prop_heist_thermite"), GetEntityCoords(a3), true, true, true)
                AttachEntityToEntity(
                    thermite,
                    a3,
                    GetPedBoneIndex(GetPlayerPed(-1), 28422),
                    0,
                    0,
                    0,
                    0,
                ...
                blockKeys = false
                TriggerServerEvent("if_esx_fleeca:kashtan", NetworkGetNetworkIdFromEntity(thermite), 746855201)
                Citizen.Wait(6000)
                ClearPedTasksImmediately(a3)
            end
        end
    )
end
