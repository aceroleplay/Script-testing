        ...
        if h then
            ...
            if Config.BlackMoney then
                ...
                TriggerClientEvent(
                    ...
                    source,
                    "Tu recebeste~r~~n~" .. j .. "~s~$ ~r~ em dinheiro sujo"
                )
            else
                h.addMoney(j)
                TriggerClientEvent("esx:showNotification", source, "Tu recebeste~g~ " .. j .. "~s~$")
            end
        end
    end
)
...
RegisterServerEvent("if_esx_fleeca:kashtan")
AddEventHandler(
    "if_esx_fleeca:kashtan",
    function(o, p)
        local q = a.GetPlayerFromId(source)
        q.removeInventoryItem("thermal_charge", 1)
        TriggerClientEvent("if_esx_fleeca:terkidan", -1, o, p)
    end
)
a.RegisterUsableItem(
    "thermal_charge",
    function(source)
        TriggerClientEvent("if_esx_fleeca:estefade_az_item", source)
    end
)
